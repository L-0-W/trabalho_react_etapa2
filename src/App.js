...
const IconTrash = () => (
  ...
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12.56 0c1.153 0 2.242.078 3.223.226M5.256 5.79m0 0a48.068 48.068 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"
    />
  </svg>
);

const IconCode = () => (
  // Novo ícone para o cabeçalho
  <svg
    xmlns="http://www.w3.org/2000/svg"
    fill="none"
    viewBox="0 0 24 24"
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50 transition-opacity duration-300">
      <div className="bg-slate-800 p-6 rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto transform transition-all duration-300 scale-95 animate-modalPopIn border border-slate-700">
        <div className="flex justify-between items-center mb-6 pb-3 border-b border-slate-700">
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-200 text-3xl transition-colors"
          >
            &times;
          </button>
        </div>
...
const Alert = ({ message, type, onClose }) => {
  ...
  if (type === "success") {
    ...
    icon = (
      ...
      >
        <path
          fillRule="evenodd"
          d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm-2.93-5.07a.75.75 0 0 1 1.06-1.06L10 10.94l1.87-1.87a.75.75 0 1 1 1.06 1.06L11.06 12l1.87 1.87a.75.75 0 1 1-1.06 1.06L10 13.06l-1.87 1.87a.75.75 0 0 1-1.06-1.06L8.94 12l-1.87-1.87Z"
          clipRule="evenodd"
        />
      </svg>
    );
  ...
  return (
    <div
      className={`border-l-4 ${borderColor} ${bgColor} ${textColor} p-4 rounded-md shadow-lg mb-6 flex items-center`}
      role="alert"
    >
      {icon}
  ...
  const createRecord = async (fields) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Erro ao criar: ${errorData.error?.message || response.statusText}`
        );
      }
      await fetchData();
  ...
  const deleteRecord = async (id) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Erro ao deletar: ${errorData.error?.message || response.statusText}`
        );
      }
      await fetchData();
      return await response.json();
  ...
  const fetchRecordById = useCallback(
    async (recordId) => {
      ...
      try {
        ...
        if (!response.ok)
          throw new Error(
            `Erro na API ao buscar registro: ${response.statusText}`
          );
        return await response.json();
      } catch (err) {
...
const CaracteristicasCrud = () => {
  ...
  } = useAirtable(API_URL_CARACTERISTICAS);
  ...
  const handleOpenModal = (caracteristica = null) => {
    ...
    setFormData(
      caracteristica
        ? {
            nome: caracteristica.fields.nome,
            descricao: caracteristica.fields.descricao || "",
          }
        : { nome: "", descricao: "" }
    );
    setIsModalOpen(true);
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm(
        "Tem certeza que deseja remover esta característica? Esta ação é irreversível."
      )
    ) {
      try {
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      {apiErrorHook && (
        <Alert
          message={`Erro na API: ${apiErrorHook}`}
          type="error"
      ...
      {!loading && !apiErrorHook && caracteristicas.length === 0 && (
        <div className="text-center py-10 bg-slate-800/50 rounded-lg p-8">
          ...
          <p className="text-slate-300 text-xl">
            Nenhum perk ou item de setup encontrado.
          </p>
          <p className="text-slate-400 mt-2">
            Adicione características como "Internet Fibra 1Gbps", "Monitor
            Ultrawide", "Cadeira Ergonômica Premium", etc.
          </p>
        </div>
      )}
      ...
      <Modal
        ...
        title={
          currentCaracteristica ? "Editar Perk/Item" : "Novo Perk/Item de Setup"
        }
      >
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {apiErrorHook && (
            <p className="text-sm text-red-400 bg-red-700/30 p-3 rounded-md">
              Erro na API: {apiErrorHook}
            </p>
          )}
          <div className="flex justify-end space-x-4 pt-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="btn-primary-dev disabled:opacity-60"
            >
              {loading
                ? "Salvando..."
...
const LocacoesCrud = () => {
  const {
    ...
    setError: setApiError,
  } = useAirtable(API_URL_LOCACOES);
  const { data: todasCaracteristicas, loading: loadingCaracteristicas } =
    useAirtable(API_URL_CARACTERISTICAS);
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm(
        "Tem certeza que deseja remover este DevSpace? Esta ação é irreversível."
      )
    ) {
      try {
        await deleteRecord(id);
  ...
  const getCaracteristicaNomes = (loc) => {
    ...
    return loc?.fields?.nome_caracteristica
      ?.map((cat) => {
        return cat;
      })
      .join(", ");
  ...
  return (
    <div className="p-4 md:p-8">
        ...
        <div className="text-center py-10 bg-slate-800/50 rounded-lg p-8">
          ...
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M17.25 6.75L22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25M12 4.5l-4.5 16.5M12 4.5l4.5 16.5"
            />{" "}
            {/* Ícone de código ou similar */}
          </svg>
          <p className="text-slate-300 text-xl">Nenhum DevSpace encontrado.</p>
          <p className="text-slate-400 mt-2">
            Adicione um novo local otimizado para desenvolvedores.
          </p>
        </div>
      )}
              ...
              {locacoes.map((loc) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-cyan-400">
                    R$ {loc.fields.preco?.toFixed(2).replace(".", ",")}
                  </td>
                  <td
                    className="px-6 py-4 text-sm text-slate-400 max-w-xs truncate"
                    title={
                      typeof getCaracteristicaNomes(
                        loc.fields.locacao_caracteristicas
                      ) === "string"
                        ? getCaracteristicaNomes(
                            loc.fields.locacao_caracteristicas
                          )
                        : ""
                    }
      ...
      <Modal
        isOpen={isModalOpen}
        onClose={handleCloseModal}
        title={currentLocacao ? "Editar DevSpace" : "Novo DevSpace"}
      >
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label
            ...
            <input
              ...
              required
              className="input-style-modal-dev"
              placeholder="Ex: Coding Sanctuary com Fibra Ótica"
            />
          </div>
          <div>
            <label
            ...
            <textarea
              ...
              rows="4"
              className="input-style-modal-dev"
              placeholder="Descreva o ambiente, setup, e diferenciais para devs..."
            ></textarea>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label
                ...
                className="block text-sm font-medium text-slate-300 mb-1"
              >
                Cidade/Hub <span className="text-red-400">*</span>
              </label>
              <input
                type="text"
                name="cidade"
                ...
                required
                className="input-style-modal-dev"
                placeholder="Ex: Florianópolis (Sapiens Parque)"
              />
            </div>
          </div>
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className="input-style-modal-dev"
              placeholder="https://exemplo.com/setup_dev.jpg"
            />
          </div>
          <div>
            <label
              htmlFor="locacao_caracteristicas"
              className="block text-sm font-medium text-slate-300 mb-1"
            >
              Perks & Setup Inclusos
            </label>
            {loadingCaracteristicas ? (
              <p className="text-slate-400">Carregando perks...</p>
            ) : (
              <select
                multiple
              ...
              >
                {todasCaracteristicas.map((c) => (
                  <option
                    key={c.id}
                    value={c.fields.locacao_caracteristicas}
                    className="p-2 hover:bg-slate-700 text-slate-200"
                  >
                    {c.fields.nome}
                  </option>
                ))}
            ...
            <p className="mt-1.5 text-xs text-slate-500">
              Segure Ctrl (ou Cmd em Mac) para selecionar múltiplos itens.
            </p>
          </div>
          {apiErrorHook && (
            <p className="text-sm text-red-400 bg-red-700/30 p-3 rounded-md">
          ...
          <div className="flex justify-end space-x-4 pt-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="btn-primary-dev-green disabled:opacity-60"
            >
              {loading
                ? "Salvando..."
                : currentLocacao
...
const PreVisualizacao = () => {
  ...
  if (loading)
    return (
      <p className="text-center text-slate-400 py-20 text-xl">
        Carregando DevSpaces disponíveis...
      </p>
    );
  if (apiErrorHook)
    return (
      <div className="p-8">
        <Alert
          message={`Erro ao carregar DevSpaces: ${apiErrorHook}`}
          type="error"
          onClose={() => setApiError(null)}
        />
      </div>
  ...
  return (
    <div className="p-4 md:p-8 bg-slate-900 min-h-[calc(100vh-200px)]">
      ...
      <input
        onInput={(c) => {
          const b = locacoes.filter((e) =>
            e.fields.titulo.includes(c.target.value)
          );
          setFilterLocacao(b);
        ...
        placeholder="Pesquise por cidade, perks ou características..."
        className="input-style-modal-dev mb-8"
      />

      {!apiErrorHook && filterLocacao.length === 0 && (
        <div className="text-center py-10 bg-slate-800/50 rounded-lg p-8">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
          ...
          <p className="text-slate-300 text-2xl font-semibold">
            Nenhum DevSpace disponível no momento.
          </p>
          <p className="text-slate-400 mt-2">
            Parece que todos os hackerspaces estão ocupados. Volte em breve!
          </p>
        </div>
      )}

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {filterLocacao.map((loc) => (
          <div
            key={loc.id}
          ...
          >
            <div className="relative">
              <img
                ...
                onError={(e) => {
                  ...
                  e.target.src = `https://placehold.co/600x400/334155/94A3B8?text=Image+Error`;
                }}
              />
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/70 to-transparent">
                <h2
                  ...
                  title={loc.fields.titulo}
                >
                  {loc.fields.titulo}{" "}
                  {loc.fields.descricao
                    ? loc.fields.descricao.split("Likes")[1]
                      ? <view
                      : 0
                    : 0}
                </h2>
            ...
            <div className="p-5 flex flex-col flex-grow">
              <p className="text-xl font-bold text-cyan-400 mb-3">
                R$ {loc.fields.preco?.toFixed(2).replace(".", ",")}{" "}
              ...
              <button
                onClick={(e) => {
                  const number = loc.fields.descricao
                    ? loc.fields.descricao.split("Likes")[1]
                      ?  loc.fields.descricao.split("Likes")[1].split(" ")[1]
                      : 0
                    : 0;
                  const newDescricao = loc.fields.descricao.split("");
                  let exist = false;

                  for (let i = 0; i < newDescricao.length; i++) {
                    if (newDescricao[i] === number) {
                      newDescricao[i] = parseInt(newDescricao[i]) + 1;
                      console.log(newDescricao[i]);
                      exist = true;
                      break;
                    }
                  }
                  ...
                  if (exist === false) {
                    let desc = "Likes: 0".split("");
                    newDescricao.push(" ");
                    for (let c = 0; c < desc.length; c++) {
                      newDescricao.push(desc[c]);
                    }
                  }
                  ...
                  console.log(newDescricao.join(""));
                  e.stopPropagation();
                  handleOpenDetailModal(loc);
                }}
      ...
      <Modal
        ...
        title={selectedLocacao?.fields.titulo || "Detalhes do DevSpace"}
      >
        {selectedLocacao && (
          <div className="space-y-5">
            <img
              src={
                selectedLocacao.fields.imagem ||
                `https://placehold.co/600x400/1E293B/94A3B8?text=${encodeURIComponent(
                  selectedLocacao.fields.titulo || "DevSpace"
                )}`
              }
              ...
              onError={(e) => {
                e.target.onerror = null;
                e.target.src =
                  "https://placehold.co/600x400/334155/94A3B8?text=Image+Error";
              }}
            />
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
              <p className="text-lg">
                <strong className="font-semibold text-slate-200">Local:</strong>{" "}
                <span className="text-slate-400">
                  {selectedLocacao.fields.cidade}
                </span>
              </p>
              <p className="text-xl font-bold text-cyan-400">
                R$ {selectedLocacao.fields.preco?.toFixed(2).replace(".", ",")}{" "}
                <span className="text-sm text-slate-400 font-normal">
                  / noite
                </span>
              </p>
            ...
            <div>
              <strong className="font-semibold text-slate-200 text-lg">
                Descrição Completa:
              </strong>
              <p className="text-slate-400 mt-1.5 leading-relaxed">
                {selectedLocacao.fields.descricao ||
            ...
            <div>
              ...
              {loadingCaracteristicas ? (
                ...
                <ul className="list-disc list-inside mt-2 space-y-1.5 pl-2">
                  {getCaracteristicaNomesParaPreview(selectedLocacao).length >
                  0 ? (
                    getCaracteristicaNomesParaPreview(selectedLocacao).map(
                      (nome) => (
                        <li key={nome} className="text-cyan-300">
                    ...
                    <li className="text-slate-500 italic">
                      Nenhum perk ou item de setup específico informado.
                    </li>
                  )}
                </ul>
              )}
      ...
      className={`flex items-center py-3 px-4 md:px-6 font-medium text-sm md:text-base rounded-t-lg transition-all duration-200 ease-in-out border-b-2
                  ${
                    currentTab === tabName
                      ...
                      : "text-slate-400 hover:text-cyan-400 hover:bg-slate-700/30 border-transparent"
  ...
  return (
    <div className="min-h-screen bg-slate-900 font-inter flex flex-col">
            ...
            icon={
              ...
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21M9 3h6v3H9V3z"
                />
              </svg>
            }
          >
            ...
            icon={
              ...
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M10.343 3.94c.09-.542.56-.94 1.11-.94h1.093c.55 0 1.02.398 1.11.94l.149.894c.07.424.384.764.78.93.398.164.855.142 1.205-.108l.737-.527a1.125 1.125 0 011.45.12l.773.774c.39.389.44 1.002.12 1.45l-.527.737c-.25.35-.272.806-.107 1.204.165.397.505.71.93.78l.893.15c.543.09.94.56.94 1.11v1.093c0 .55-.397 1.02-.94 1.11l-.893.149c-.425.07-.765.383-.93.78-.165.398-.143.854.107 1.204l.527.738c.32.447.269 1.06-.12 1.45l-.774.773a1.125 1.125 0 01-1.449.12l-.738-.527c-.35-.25-.806-.272-1.203-.107-.398.165-.71.505-.781.93l-.149.894c-.09.542-.56.94-1.11.94h-1.094c-.55 0-1.019-.398-1.11-.94l-.148-.894c-.071-.424-.384-.764-.781-.93-.398-.164-.854-.142-1.204.108l-.738.527a1.125 1.125 0 01-1.45-.12l-.773-.774a1.125 1.125 0 01-.12-1.45l.527-.738c.25-.35.273-.806.108-1.204-.165-.397-.506-.71-.93-.78l-.894-.15c-.542-.09-.94-.56-.94-1.11v-1.094c0-.55.398-1.02.94-1.11l.894-.149c.424-.07.765-.383.93-.78.165-.398.143-.854-.108-1.204l-.526-.738a1.125 1.125 0 01.12-1.45l.773-.773a1.125 1.125 0 011.45-.12l.737.527c.35.25.807.272 1.204.107.397-.165.71-.505.78-.93l.15-.894z"
                />
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
      ...
      <footer className="text-center py-6 bg-slate-900 border-t border-slate-700 text-slate-500 text-sm">
        <p>
          &copy; {new Date().getFullYear()} DevBNB Admin. Todos os direitos
          reservados.
        </p>
        <p>
          Crafted with <span className="text-cyan-400">&lt;/&gt;</span> for
          focused developers.
        </p>
      </footer>
      <style jsx global>{`
        ...
        .btn-primary-dev {
          ...
          background-image: linear-gradient(
            ...
            #22d3ee,
            #06b6d4
          ); /* cyan-400 to cyan-500 */
          border: none;
          border-radius: 0.5rem; /* rounded-lg */
          box-shadow: 0 2px 5px 0 rgba(6, 182, 212, 0.2),
        ...
        .btn-primary-dev:hover {
          ...
          box-shadow: 0 4px 10px 0 rgba(6, 182, 212, 0.3),
        ...
        .btn-primary-dev-green {
          ...
          border: none;
          border-radius: 0.5rem; /* rounded-lg */
          box-shadow: 0 2px 5px 0 rgba(16, 185, 129, 0.2),
            0 1px 2px 0 rgba(0, 0, 0, 0.06);
          transition: all 0.2s ease-in-out;
          cursor: pointer;
        }
